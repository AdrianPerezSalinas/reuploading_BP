...
def main(Qubits):
    ...
    for qubits in range(2, Qubits, 2):
        ...
        reuploading_params = 2 * np.pi * np.random.rand(3 * qubits)

        ansatz = Circuit(qubits)
        for l in range(qubits):
            for q in range(qubits):
                ansatz.add(gates.RZ(q, theta=reuploading_params[3 * q], trainable=False))
                ansatz.add(gates.RY(q, theta=reuploading_params[3 * q + 1], trainable=False))
                ansatz.add(gates.RZ(q, theta=reuploading_params[3 * q + 2], trainable=False))
        ...
        ham = hamiltonians.XXZ(qubits, delta=h)
    ...
    ax.set_yscale('log')
    fig.savefig('RZYZ-reuploading-XXZ.pdf')
