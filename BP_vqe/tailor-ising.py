...
def main(Qubits):
    ...
    for qubits in range(2, Qubits, 2):
        ...
        for q in range(qubits):
            ansatz.add(gates.H(q))
        for l in range(qubits):
            for q in range(qubits):
                ansatz.add(gates.RX(q, theta=0))
            for q in range(0, qubits - 1, 2):
                ansatz.add(gates.CNOT(q, q + 1))
            ...
            for q in range(1, qubits - 1, 2):
                ...
                ansatz.add(gates.RZ(q, theta=0))
                ansatz.add(gates.CNOT(q, q + 1))
            ansatz.add(gates.CNOT(qubits - 1, 0))
            ansatz.add(gates.RZ(0, theta=0))
            ansatz.add(gates.CNOT(qubits - 1, 0))
